	...
	async fetchArticles(filters?: ArticleFilters) {
		...
		try {
			const params: ListArticlesParams = {
				...
				topic: filters?.topic,
				is_read: filters?.is_read !== undefined ? (filters.is_read ? 'true' : 'false') : undefined,
				is_saved: filters?.is_saved !== undefined ? (filters.is_saved ? 'true' : 'false') : undefined,
				is_archived: filters?.is_archived !== undefined ? (filters.is_archived ? 'true' : 'false') : undefined,
